...
| `MessageThread` | 保存已接受连接对应的消息线程。 | `connectionId`、`permissionStatus`、`lastMessageAt`、`createdAt` |
| `Message` | 保存已接受连接线程中的消息。 | `threadId`、`senderId`、`body`、`createdAt`、`readAt` |
...
`MessageThread` 是消息功能的实现层实体。页面路由使用 `/messages/[threadId]`，但线程可访问性不能只依赖 `permissionStatus` 字段；服务端权限辅助函数和 RLS 必须通过对应 `Connection` 判断双方是否处于已接受连接关系。
...
`Visibility.public` 是为后续公开展示或跨校园扩展预留的状态。第一版 MVP 的界面不开放用户选择 `public`，默认可见性应使用 `campus`，私密草稿或不应被发现的内容使用 `private`。如果未来要在 UI 中开放 `public`，应先重新评审访问边界、RLS 和产品说明。
...
| `MessageThread` | 已连接双方 | 只能由已接受连接创建或读取 |
