...
During the latest AI/recommendation workstream update, the shared AI boundary was tightened further with a unified `apps/web/src/server/ai` export surface, output-structure validation, transparent recommendation scoring, a minimal core recommendation path, event-recording helpers, persisted recommendation cards, and OpenAI provider activation. Demo seed data now exercises the first loop from posts and profiles through recommendations, accepted connections, and messages.
