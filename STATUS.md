...
| Authentication and onboarding gates | `/auth` now uses Supabase campus email link sign-in and callback handling, demo auth bypass is restricted to local development or explicit Playwright E2E runs, and protected pages/actions now redirect missing sessions to `/auth?next=...` or incomplete profiles to the onboarding gate. | Add hosted Supabase email-template/configuration checks once the deployment target is selected. |
