...
| MVP planning | Product loop is defined, but implementation scope, data model, user flows, AI boundaries, and verification process are not yet specified. | Techno MVP implementation guide. |
| User validation | Around 60 early survey responses exist, but they should be treated as early signal rather than conclusive evidence. | Descriptive analysis plus targeted interviews with Math, CS, and EEE students. |
...
| Recommendation logic | AI-assisted recommendation is in scope, but the first version of signals and explanation logic is not yet defined. | MVP recommendation rules and prompt/data requirements. |

## Near-Term Execution Sequence

The Biz reference folder has now been initiated. It includes guidance for business-plan structure, CVP, USP, revenue-model hypotheses, survey-data usage, market validation, and pitch narrative. The next comparable document set should be the Techno MVP implementation guide.

The Biz team should use the existing survey responses as early evidence and conduct targeted interviews with Math, CS, and EEE students. In parallel, the Techno team should define the MVP information architecture, data model, authentication approach, profile flow, post flow, recommendation flow, and accepted-only messaging flow.
...
When code is reintroduced, the Techno MVP implementation guide should define the verification commands required before delivery.
